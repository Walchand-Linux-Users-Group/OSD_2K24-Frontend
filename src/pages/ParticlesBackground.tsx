...
const Particle: React.FC<{ position: [number, number, number]; size: number }> = ({ position, size }) => {
  ...
  const velocity = useRef(new Vector3(Math.random() * 0.02 - 0.01, Math.random() * 0.02 - 0.01, 0)); // Random velocity
...
const Particles: React.FC = () => {
  const particleCount = 120; // More particles for a denser background
